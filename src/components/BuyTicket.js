...
export default function BuyTicket({ signer }) {
      ...
      try {
        ...
        for (let i = 0; i < movieCount; i++) {
          ...
          arr.push({
            id: i,
            title: movie[0],
            price: BigInt(movie[1]),
            seats: Number.parseInt(movie[2]),
          })
        }
        setMovies(arr)
      } catch (err) {
  ...
  const buyTicket = async () => {
    ...
    const index = Number.parseInt(movieId)
    const ticketCount = Number.parseInt(count)
    ...
    try {
      ...
      const pricePerTicket = BigInt(movie[1])
      const total = pricePerTicket * BigInt(ticketCount)
      ...
      const totalSeats = Number.parseInt(movie[2])
      const startSeat = totalSeats - ticketCount + 1
      const seatNumbers = Array.from({ length: ticketCount }, (_, i) => startSeat + i)
  ...
  return (
    <div className="card">
      <div className="card-header">
        ...
        <p className="card-subtitle">Select a movie and number of tickets</p>
      </div>
      ...
      <div className="card-body">
        ...
        <button className="btn btn-primary btn-block" onClick={buyTicket} disabled={loading}>
          ...
          <div className="purchase-details">
            ...
            <div className="purchase-info">
              <div className="purchase-row">
                ...
                <span className="value">{purchase.title}</span>
              </div>
              <div className="purchase-row">
              ...
              <div className="purchase-row">
                ...
                <span className="value">{purchase.count}</span>
              </div>
              <div className="purchase-row">
                <span className="label">Seats:</span>
                <span className="value">{purchase.seats.join(", ")}</span>
              </div>
            </div>
          </div>
        )}
